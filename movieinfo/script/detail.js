...
$like.addEventListener("click", async (e) => {
	...
	if (await response.ok) {
		...
		const response = await fetch(
			`${backend_url}detail/${movie}/like/userlike/${decodeJwt(localStorage.getItem("access_token")).payload.user_id}/`,
			{
				method: "DELETE",
				headers: {
					"Content-Type": "application/json",
...
$watched.addEventListener("click", async (e) => {
	...
	if (await response.ok) {
		...
		const response = await fetch(
			`${backend_url}detail/${movie}/watched/userlike/${decodeJwt(localStorage.getItem("access_token")).payload.user_id}/`,
			{
				method: "DELETE",
				headers: {
					"Content-Type": "application/json",
...
$watchlist.addEventListener("click", async (e) => {
	...
	if (await response.ok) {
		...
		const response = await fetch(
			`${backend_url}detail/${movie}/watchlist/userlike/${decodeJwt(localStorage.getItem("access_token")).payload.user_id}/`,
			{
				method: "DELETE",
				headers: {
					"Content-Type": "application/json",
...
fetch(`${backend_url}detail/${movie}/onelinecritic/`, {
	...
	.then((datas) => {
		...
		datas.forEach((data) => {
			try {
				const jwt_id = decodeJwt(localStorage.getItem("access_token")).payload.user_id;
